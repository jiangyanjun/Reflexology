    ...
    public class hr_position : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "grid")
            {
                ...
                DataSet ds = zw.GetList(0, " ISNULL(isDelete,0)=0 ", "convert(int,[position_order])");
                string dt = Common.GetGridJSON.DataTableToJSON(ds.Tables[0]);
                context.Response.Write(dt);
            }
            if (request["Action"] == "delgrid")
            {
                DataSet ds = zw.GetList(0, " ISNULL(isDelete,0)=1 ", " convert(int,[position_order])");
                string dt = Common.GetGridJSON.DataTableToJSON(ds.Tables[0]);
                context.Response.Write(dt);
            }
            ...
            if (request["Action"] == "AdvanceDelete")
            {
                ...
                {
                    bool isdel = zw.AdvanceDelete(int.Parse(request["id"]), 1, DateTime.Now.ToString(StaticStr.yyyy_MM_dd_HH_mm_ss));
                    if (isdel)
                    {
            ...
            if (request["Action"] == "regain")
            {
                ...
                {
                    zw.AdvanceDelete(int.Parse(arr[i]), 0, DateTime.Now.ToString(StaticStr.yyyy_MM_dd_HH_mm_ss));
                }
