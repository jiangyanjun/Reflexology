...
namespace Kebue.Core.Database
{
    public class DBHelper
    {
        ...
        /// <param name="p">参数集合</param>
        /// <returns>对象集合</returns>
        public static IEnumerable<T> ReadCollectionBySP<T>(string procName, DynamicParameters p) where T : class
        {
            using (ConnService connService = new ConnService())
            {
        ...
        /// <param name="p">参数集合</param>
        /// <returns>对象集合</returns>
        public static IEnumerable<T> ReadCollectionBySP<T>(string procName, ref DynamicParameters p) where T : class
        {
            using (ConnService connService = new ConnService())
            {
                return connService.Connection.Query<T>(procName, p, commandType: CommandType.StoredProcedure).ToList<T>();
        ...
        public static int SaveCollection<T>(string sql, IEnumerable<T> collection)
        {
            using (ConnService connService = new ConnService())
            {
        ...
        //public static DataSet GetDataSet(string sql, DynamicParameters p)
        //{
        //    using (ConnService conn = new ConnService())
        //    {
        ...
        //            if (p != null)
        //            {
        //                dt.Load(conn.Connection.ExecuteReader(sql, p));
        //            }
        //            else
        //            {
        //                dt.Load(conn.Connection.ExecuteReader(sql));
        //            }
        //            ds.Tables.Add(dt);
        //        }
        //        return ds;
        //    }
        //}
        public static DataSet GetDataSet(string sql, DynamicParameters ps)
        {
            DataSet ds = new DataSet();
            using (ConnService conn = new ConnService())
            {
                ...
                {
                    //var s = conn.DbCommand();
                    //cmd.Connection = (SqlCommand)conn.;
                    cmd.CommandText = sql;
        ...
        /// <param name="sql">sql语句,示例 insert AUT_User (Name,Age) values(@Name, @Age)</param>
        /// <param name="collection">集合对象</param>
        public static void SaveCollection<T>(string sql, IEnumerable<T> collection, ConnService connService, IDbTransaction transaction)
        {
